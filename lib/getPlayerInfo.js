async function getPlayerInfo(playerId, player, nations, teamData, allPlayerPosData, playerStatsPerId, playerStatsRaw, rsmData) {
    try {
        ...
        const team = teamData.teams.find(team => team.teamId === player.team)
        let rsmTeam = rsmData.find(t => t.id === team.rsmId)

        player.teamLogo = team.logo
        player.teamName = team.teamname
        if (rsmTeam) {
            let rsmPlayerData = rsmTeam.players.find(p => p.id == player.rsmId)
            player.avatar = rsmPlayerData?.avatarURL || "/images/defaulticon.png";
            player.rating = rsmPlayerData?.rating || null;
            player.username = rsmPlayerData?.username || null;
        } else {
            player.avatar = "/images/defaulticon.png"
            player.rating = null
            player.username = null
        }

        player.career = player?.career?.map(t => {
            let team = teamData.teams.find(team => team.teamId === t.team)
            return {
                ...t,
        ...
        player.teamColors = {}
        if (team.color) {
            player.teamColors.teamColorMain = team.color
            player.teamColors.teamColorAlternate = isDark(player.teamColors.teamColorMain) === true ? "rgb(255, 255, 255)" : team.color
        } else {
        ...
        function playerPercentiles(playerId, playerTotalStats, playerPool) {
            ...
            function competitionStatsObj(num) {
                ...
                if (competitionStatsMetrics[num] === "gkSavePercentage") {
                    (competitionStatsItem.value = (playerTotalStats[competitionStatsMetrics[num]] * 100).toFixed(1) + '%')
                } else {
