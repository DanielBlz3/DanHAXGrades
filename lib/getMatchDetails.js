...
import roundMap from '/lib/RoundMap';
    ...
    function gameStatusTest() {
        if (status.awarded === true) {
            ...
            if (matchData.timestamp) {
                status.statusShort = {date: formatMatchTimestamp(matchData.timestamp).date, time: formatMatchTimestamp(matchData.timestamp).time};
            } else {
                status.statusShort = "";
            }
        }
    ...
    class lineup {
        constructor(teamSide) {
            ...
            players.map((player, i) => {
                ...
                const {
                    assists, keyPasses, errors, bigErrors, ownGoals,
                    duelsWon, duelsLost, saves, goalsConceded,
                    gkPunches, gkRecovery, actedAsSweeper, bonus,
                    shots, goals, secondAssists, defensiveActions,
                    lastManTackles, clearancesOffTheLine, passesC, bcm
                } = stats || {};
                ...
                if (playerData.find(p => p.playerId === playerId)) {
                    ...
                    const { matchRating, matchRatingRounded } = getMatchRating(isGK);
