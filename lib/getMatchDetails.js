...
async function getMatchDetails(matchId, leagueId, teamData, matchData, playerData, nationData) {
    ...
    general.roundNumber = matchData.roundNumber
    ...
    class lineup {
        constructor(teamSide) {
            ...
            players.map((player, i) => {
                ...
                if (playerData.find(p => p.id === playerId)) {
                    const playerDetails = playerData.find(p => p.id === playerId)
                    const playerNation = playerDetails.nation;
    ...
    function renderTimeline(data) {
        if (data) {
            const timeline = data.map(i => {
                const item = {}
                item.type = i.type
                item.minute = i.minute
                item.team = i.team
                if (i.type === "goal") {
                    item.goalType = i.goalType

                    item.goalscorerId = i.goalscorerId
                    const goalPlayer = playerData?.find(p => p.playerId === i.goalscorerId)
                    item.goalscorerName = goalPlayer?.playername
                    item.goalscorerPageUrl = "/players/" + i.goalscorerId

                    item.assisterId = i.assisterId
                    const assistPlayer = playerData?.find(p => p.playerId === i.assisterId)
                    item.assisterName = assistPlayer?.playername
                    item.assisterPageUrl = "/players/" + i.assisterId
                }
                return (
                    item
                )
    ...
    const timeline = renderTimeline(matchData.timeline)
