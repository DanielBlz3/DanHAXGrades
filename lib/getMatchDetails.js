        ...
        if (data) {
            const timeline = data.map(i => {
                ...
                if (i.type === "goal") {
                    ...
                    const goalPlayer = playerData?.find(p => p.id === i.goalscorerId)
                    item.goalscorerName = goalPlayer?.name
                    item.goalscorerPageUrl = "/players/" + i.goalscorerId

                    item.assisterId = i.assisterId
                    const assistPlayer = playerData?.find(p => p.id === i.assisterId)
                    item.assisterName = assistPlayer?.name
