...
async function getLeagueDetails(leagueId, leagueData, playerData, fixturesData, leagueTableData, exanonStatsData, playerStatsPerId) {
    ...
    function playerStats() {
        ...
        const minforPer20Stats = leagueInfoObj.leagueDesc.minforPer20Stats
        function getTopPlayersByStat(data, statKey, topN = 3) {
            return data
                ...
                .map(player => ({
                    ...
                    pageUrl: '/players/' + player.playerId,
                    avatar: playerData.find(p => p.id === player.playerId)?.avatar,
                    playerName: playerData.find(p => p.id === player.playerId)?.name,
        ...
        const topPassingAccuracy = individualPlayerGameStatsPerId.filter(player => player.minutes >= minforPer20Stats).sort((a, b) => b.passingA - a.passingA).slice(0, 3).map(player => ({
            playerId: player.playerId,
            pageUrl: '/players/' + player.playerId,
            avatar: playerData.find(p => p.id === player.playerId)?.avatar,
            playerName: playerData.find(p => p.id === player.playerId)?.name,
        ...
        const topDuelsWonPercentage = individualPlayerGameStatsPerId.filter(player => player.duelsWon >= 7).sort((a, b) => b.duelsP - a.duelsP).slice(0, 3).map(player => ({
            playerId: player.playerId,
            pageUrl: '/players/' + player.playerId,
            avatar: playerData.find(p => p.id === player.playerId)?.avatar,
            playerName: playerData.find(p => p.id === player.playerId)?.name,
        ...
        const topsavePercentage = individualPlayerGameStatsPerId.filter(player => player.saves >= 5).sort((a, b) => b.gkSavePercentage - a.gkSavePercentage).slice(0, 3).map(player => ({
            playerId: player.playerId,
            pageUrl: '/players/' + player.playerId,
            avatar: playerData.find(p => p.id === player.playerId)?.avatar,
            playerName: playerData.find(p => p.id === player.playerId)?.name,
        ...
        function statToPer20(metric) {
            ...
            topStatsObj[metric] = individualPlayerGameStatsPerId.filter(player => player.minutes >= minforPer20Stats).sort((a, b) => b[`${metric}Per20`] - a[`${metric}Per20`]).slice(0, 3).map(player => ({
                playerId: player.playerId,
                pageUrl: '/players/' + player.playerId,
                avatar: playerData.find(p => p.id === player.playerId)?.avatar,
                playerName: playerData.find(p => p.id === player.playerId)?.name,
        ...
        var topmatchRatings = individualPlayerGameStatsPerId.filter(player => player.minutes >= leagueInfoObj.leagueDesc.minforPer20Stats)
        ...
        function tots(matchday, totsArr) {
            const playerGameStats = matchday == "matchdayTOTS" ? individualPlayerGameStatsPerId :
                exanonStatsData.filter(p => p.roundNumber == matchday)
            ...
            if (matchday == "matchdayTOTS") {
                ...
                function totwFilterForPosition(allowedPos, maxPlayers) {
                    return playerGameStats.filter(player => allowedPos.includes(player.position)).slice(0, maxPlayers).map(player => ({
                        ...
                        teamLogo: playerData.find(p => p.id === player.playerId)?.teamLogo,
            ...
            function findTotsPlayers() {
                ...
                const sortedLineup = [...totsLineup].sort((a, b) => b.matchRating - a.matchRating);
                const mvp = sortedLineup[0]; for (let player = 0; player < totsLineup.length; player++) {
                    if (totsLineup[player]) {
                        totsLineup[player].coords = {
                            x: totsCoords[player][0],
                            y: totsCoords[player][1],
                        }
                        if (totsLineup[player] === mvp) {
                            totsLineup[player].isMvp = true
                        } else {
                            totsLineup[player].isMvp = false
                        }
                    }
                }
            }
