...
async function getLeagueDetails(leagueId, leagueData, playerData, fixturesData, leagueTableData, exanonStatsData, playerStatsPerId) {
    ...
    function playerStats() {
        ...
        const topPassingAccuracy = individualPlayerGameStatsPerId.filter(player => player.minutes >= leagueInfoObj.leagueDesc.minforPer20Stats).sort((a, b) => b.passingA - a.passingA).slice(0, 3).map(player => ({
            playerId: player.playerId,
            pageUrl: '/players/' + player.playerId,
            avatar: playerData.find(p => p.id === player.playerId)?.avatar,
            playerName: playerData.find(p => p.id === player.playerId)?.name,
        ...
        function statToPer20(metric) {
            ...
            topStatsObj[metric] = individualPlayerGameStatsPerId.filter(player => player.minutes >= leagueInfoObj.leagueDesc.minforPer20Stats).sort((a, b) => b[`${metric}Per20`] - a[`${metric}Per20`]).slice(0, 3).map(player => ({
                playerId: player.playerId,
                pageUrl: '/players/' + player.playerId,
                avatar: playerData.find(p => p.id === player.playerId)?.avatar,
                playerName: playerData.find(p => p.id === player.playerId)?.name,
