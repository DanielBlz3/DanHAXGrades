    ...
    try {
        ...
        for (let i = 1; i < playerData.length + 1; i++) {
            ...
            for (const metric in playerTotalStats) {
                if (!["playerPosition", "playerId", "passingA", "duelsP", "gkSavePercentage", "teamId", "teamName", "teamLogo", "teamColor"].includes(metric)) {
            ...
            playerTotalStats.appearances = focusedPlayer.length
            allPlayerStats.push(playerTotalStats);
        }
        allPlayerStats.forEach(p => {
            if (p.appearances > 0) {
                p.matchRating = Math.round((p.matchRating / p.appearances) * 100) / 100;
            } else {
                p.matchRating = 6.0;
            }
        });
