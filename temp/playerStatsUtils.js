...
const playerPositionsUrl = 'http://danhaxgrades.glitch.me/api/allPlayerPos';
const playerStatsRawUrl = 'https://danhaxgrades.glitch.me/api/leagueStats/';
const playerStatsPerIdUrl = 'https://danhaxgrades.glitch.me/api/playerStatsPerId/';

async function getPlayerInfo(playerData, playerId, player, exanonLeagueData, exanonCupData, nations, teamObj) {
    try {
        const playerPosRes = await fetch(playerPositionsUrl);
        if (!playerPosRes.ok) {
            const errorText = await playerPosRes.text();
            console.error(`Error fetching data: ${playerPosRes.status} - ${errorText}`);
            throw new Error(`HTTP error! status: ${playerPosRes.status}`);
        }
        const allPlayerPosData = await playerPosRes.json()

        const playerStatsPIdRes = await fetch(playerStatsPerIdUrl);
        if (!playerStatsPIdRes.ok) {
            const errorText = await playerStatsPIdRes.text();
            console.error(`Error fetching data: ${playerStatsPIdRes.status} - ${errorText}`);
            throw new Error(`HTTP error! status: ${playerStatsPIdRes.status}`);
        }
        const playerStatsPerId = await playerStatsPIdRes.json()
       
        const playerStatsRawRes = await fetch(playerStatsRawUrl);
        if (!playerStatsRawRes.ok) {
            const errorText = await playerStatsRawRes.text();
            console.error(`Error fetching data: ${playerStatsRawRes.status} - ${errorText}`);
            throw new Error(`HTTP error! status: ${playerStatsRawRes.status}`);
        }
        const playerStatsRaw = await playerStatsRawRes.json()
...
//
        ...
        function playerPercentiles(playerId, playerInfo, playerTotalStats, playerPool) {
            var statData = {};
            var playerRatings = [{}, {}, {}, {}, {}];
        ...
        if (mainPos) {
            var percentileAndRating = playerPercentiles(playerId, player, focusedPlayerTotalStats, playerGameStatsFormattedFP)
